...
def validate_excel(filepath, file_type, category=None, file_idx=0):
    print(
        f"validate_excel: Entering function with filepath={filepath}, file_type={file_type}, category={category}, file_idx={file_idx}")
    try:
        pre_headers = [
        ...
        if file_type == 'pre_disbursement':
            ...
            if category == 'los':
                ...
                print(
                    f"validate_excel: Set expected_headers to {'los_headers' if file_idx == 0 else 'mis_headers'} for 'both'")
            else:
...
def handle_upload():
    ...
    print(
        f"handle_upload: file_type={file_type}, category={category}, files={len(filepaths)}, has_pre={has_pre}, has_post={has_post}, input_timestamp={input_timestamp}")
...
def process_upload():
    ...
    print(
        f"process_upload: file_type={file_type}, category={category}, files={len(filepaths)}, input_timestamp={input_timestamp}")
    duplicates = {}
    new_records = {}
    summary_path = None
    ...
    try:
        ...
        for file_idx, filepath in enumerate(filepaths):
            ...
            for sheet_name in sheet_names:
                ...
                if df.columns.duplicated().any():
                    print(
                        f"process_upload: Found duplicate columns in sheet '{sheet_name}': {df.columns[df.columns.duplicated()].tolist()}")
                    raise ValueError(
                ...
                if file_type == 'pre_disbursement':
                    ...
                    print(
                        f"process_upload: Set pre_disbursement settings: table_name={table_name}, key_column={key_column}")
                    ...
                    key_column = 'loan_no'
                    print(
                        f"process_upload: Set post_disbursement settings: table_name={table_name}, key_column={key_column}")
                    ...
                    if category == 'los' or (category == 'both' and file_idx == 0):
                        ...
                        if category == 'both':
                            ...
                            print(
                                f"process_upload: Applied los_to_mis_mapping for 'both', new columns: {list(df.columns)}")
                ...
                if key_column not in df.columns:
                    print(
                        f"process_upload: Key column '{key_column}' not found in sheet '{sheet_name}', columns={list(df.columns)}")
                    raise ValueError(
                        f"Key column '{key_column}' not found in sheet {sheet_name} after mapping. Available columns: {list(df.columns)}")

                print(
                    f"process_upload: Processing sheet '{sheet_name}' with key_column={key_column}, columns={list(df.columns)}")
                ...
                print(
                    f"process_upload: Converted new_records for sheet '{sheet_name}' to DataFrame with {len(new_records[sheet_name])} rows")
        ...
        if file_type == 'post_disbursement' and category == 'both':
            ...
            if all_dfs:
                ...
                print(
                    f"process_upload: Combined {len(all_dfs)} DataFrames into combined_df with {len(combined_df)} rows")
