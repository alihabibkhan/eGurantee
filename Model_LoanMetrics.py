...
def get_all_loan_metrics():
    query = f"""
        SELECT lm.loan_metric_id, lp.name as product_name, lp.gender, o.name as occupation_name,
               er.label as experience_label, b.branch_name as branch_name, lm.global_loan_ceiling,
               lm.repeat_increment, lm.is_active, lm.status, uc.name as created_by, um.name as modified_by, lm.created_date, lm.modified_date
